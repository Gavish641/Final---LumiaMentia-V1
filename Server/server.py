...
def handle_client(client_socket):
    """
    Function to handle a client connection and receive messages until the client sends an exit signal.
...
def client_exit(client_socket):
    """
    Closes the client socket and prints a message indicating that the client has disconnected.
    Parameters:
        client_socket: the client socket to be closed
    """
    client_socket.close()
            ...
            if type(msg) is list:
                ...
                if msg[0] == "database":
                    ...
                    if msg[1] == "check remember me status":
                        ...
                        self.messages.pop(sock)
                        return [json.dumps(self.encryption.encrypt(str(0))), json.dumps(self.encryption.encrypt(str(0)))]
                ...
                if msg[0] == "game":
                    ...
                    if msg[1] == "sorting numbers":
                        ...
                        if msg[2] == "set score":
                            ...
                            self.messages.pop(sock)
                            encrypted_score = self.encryption.encrypt(str(score))
